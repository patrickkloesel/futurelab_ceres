...
ccmt <- readRDS(".\\results\\ccmt_IIS.RDS") %>% 
...
ccmt_models <- rbind(ccmt, ccmt_noiis)
...
```{r, echo=FALSE}
energy <- readRDS(".\\results\\energy_IIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("TRUE", iis) ~ "Model w IIS")) 

noiis_energy <- readRDS(".\\results\\energy_noIIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("FALSE", iis) ~ "Model w/o IIS"))

models_energy <- rbind(energy, noiis_energy)
...
```{r, echo=FALSE}
solar <- readRDS(".\\results\\solar_IIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("TRUE", iis) ~ "Model w IIS")) 

noiis_solar <- readRDS(".\\results\\solar_noIIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("FALSE", iis) ~ "Model w/o IIS"))

models_solar <- rbind(solar, noiis_solar)
...
```{r, echo=FALSE}
wind <- readRDS(".\\results\\wind_IIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("TRUE", iis) ~ "Model w IIS")) 

noiis_wind <- readRDS(".\\results\\wind_noIIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("FALSE", iis) ~ "Model w/o IIS"))

models_wind <- rbind(wind, noiis_wind)
...
```{r, echo=FALSE}
batteries <- readRDS(".\\results\\batteries_IIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("TRUE", iis) ~ "Model w IIS")) 

noiis_batteries <- readRDS(".\\results\\batteries_noIIS.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula),
         mod_name = case_when(grepl("FALSE", iis) ~ "Model w/o IIS"))

models_batteries <- rbind(batteries, noiis_batteries)
