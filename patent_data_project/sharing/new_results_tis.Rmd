...
## Models comparison 06.03 version {.tabset}
...
## 22.03 version {.tabset}

 * IIS=OFF

```{r models22, echo=FALSE, results = 'asis', fig.height=40, fig.width=60, cache = TRUE}

res22 <- readRDS("C:\\Users\\laura\\OneDrive\\Documenti\\LAURA\\MCC\\futurelab_ceres\\patent_data_project\\results\\22_03_TIS_nobrown_notrends.RDS") %>% 
  separate(source, into = c("dep","formula"), sep = "[~]", remove = FALSE) %>%
  mutate(dep = str_to_title(gsub("_"," ",trimws(dep))),
         formula = trimws(formula), 
         tech = word(dep, -1, sep = " "), 
         iis = FALSE) # here change to FALSE because of wrong tibble specification in run_model.R
